...
// userCommands are commands that non-admin users are allowed to use.
// All other commands are blocked for non-admin users.
...
func (e *Engine) handleCommand(p Platform, msg *Message, raw string) bool {
	...
	// Non-admin users can only use a restricted set of commands
	if cmdID != "" && !e.isAdmin(msg.UserID) && !userCommands[cmdID] {
		slog.Info("audit: command_blocked",
			"user_id", msg.UserID, "platform", msg.Platform,
			"project", e.name, "command", cmdID, "reason", "not_in_user_allowlist")
		e.reply(p, msg.ReplyCtx, fmt.Sprintf(e.i18n.T(MsgCommandDisabled), "/"+cmdID))
		return true
	}
